...
def process_files():
    ...
    user_ids = pd.read_csv('data/diverse_users.csv')['user_id'].tolist()
    files = [f for f in os.listdir(folder_path) if f.endswith('.csv')]

    aggregated_results = []
    
    for file in files:
        user_id = int(file.split('.')[0][1:])
        if user_id not in user_ids:
            continue

        print(f"Processing file: {file}")
        df = pd.read_csv(os.path.join(folder_path, file), nrows=100)  # Limit to 100 samples per file
