...
def test_policy(env, policy_network):
    """
    Test the policy network on the provided environment.
    """
    total_reward = 0
    ...
    for idx in range(env.num_states):  # Each state in the dataset
        ...
        total_reward += reward
        if reward == 1:  # Correct prediction
...
def test_files():
    ...
    for file in files:
        ...
        model_path = f"models/a2_100s/trained_policy_{file}.pth"
        try:
            policy_network.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
            print(f"Loaded model from {model_path}")
        except FileNotFoundError:
            #print(f"Model not found for {file}. Skipping...")
            continue
        except Exception as e:
            #print(f"Error loading model for {file}: {e}. Skipping...")
            continue
    ...
    results_df.to_csv("test_results_summary_a2_100s.csv", index=False)
    print("Test results saved to test_results_summary_a2_100s.csv")
